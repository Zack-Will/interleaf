...
const defaults = { ProjectRef, SnapshotService, VersionService, LabelService, WriteService, ProjectGetter }
...
export function registerTools(server, { services = defaults, req = {}, clientName } = {}) {
  ...
  server.tool('get_project', 'Get project metadata', { project: z.string() }, async ({ project }) => run(async () => {
    ...
    const root = p?.rootDoc_id ? String(p.rootDoc_id) : tree.find(x=>x.kind==='doc')?.path
  ...
  server.tool('read_file', 'Read a text file', { project: z.string(), path: z.string(), start_line: z.number().optional(), end_line: z.number().optional() }, async ({ project, path, start_line, end_line }) => run(async () => {
    ...
    return { path:d.path, content:lines, lines:d.lines, project_version:d.docVersion, sha256:d.sha256, total_lines:d.totalLines }
  }))
  ...
  server.tool('list_history', 'List project history', { project: z.string(), limit: z.number().optional() }, async ({ project, limit=50 }) => run(async () => {
    ...
    return [...labels.map(l=>({version:l.version,comment:l.comment,label:l,users:l.user_id?[l.user_id]:[],timestamp:l.timestamp,origin:{kind:'label'}})), ...arr].sort((a,b)=>new Date(b.timestamp||0)-new Date(a.timestamp||0)).slice(0,limit)
  }))
