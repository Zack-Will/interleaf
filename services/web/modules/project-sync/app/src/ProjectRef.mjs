...
function parse(input) {
  ...
  if (/^[0-9a-fA-F]{24}$/.test(input))
    return { projectId: input.toLowerCase() };
  let pathname;
  try {
...
function urlFor(projectId) {
  return `${Settings.siteUrl}/project/${projectId}`;
}
async function requireAccess(userId, projectId, level) {
  const { default: AuthorizationManager } =
    await import("../../../../app/src/Features/Authorization/AuthorizationManager.mjs");
  const allowed =
    level === "write"
  ...
  const { default: ProjectGetter } =
    await import("../../../../app/src/Features/Project/ProjectGetter.mjs");
