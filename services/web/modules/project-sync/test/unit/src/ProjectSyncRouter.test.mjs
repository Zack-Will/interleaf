...
describe('project-sync router', () => {
  ...
  it('registers kebab-case personal access token routes', async ctx => {
    const paths = [
      ctx.webRouter.get.firstCall.args[0],
      ctx.webRouter.post.firstCall.args[0],
      ctx.webRouter.delete.firstCall.args[0],
    ]
  ...
  it('returns 404 when revoking an unknown token', async ctx => {
    ...
    const revokeToken = (await import('../../../app/src/TokenService.mjs'))
      .default.promises.revokeToken
    revokeToken.resolves({ deletedCount: 0 })
    const res = { sendStatus: sinon.stub() }
    await deleteHandler(
      { params: { tokenId: 'missing' }, session: {} },
      res,
      sinon.stub()
    )
