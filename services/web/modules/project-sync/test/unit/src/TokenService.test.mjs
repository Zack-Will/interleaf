...
describe('project-sync TokenService', () => {
  ...
  it('rejects invalid creation options with invalid_request errors', async ctx => {
    const { InvalidTokenRequestError } =
  ...
  it('verifies a token and records last use asynchronously', async ctx => {
    ...
    const result = await ctx.TokenService.promises.verifyToken(
      token,
      'git_bridge'
    )
  ...
  it('rejects unknown, malformed, expired, and insufficient-scope tokens', async ctx => {
    await expectRejection(
      ctx.TokenService.promises.verifyToken('bad', 'git_bridge'),
      { code: 'token_invalid' }
    )
    const token = 'olp_1234567890abcdef'
    ctx.PersonalAccessToken.findOne.resolves(null)
    await expectRejection(
      ctx.TokenService.promises.verifyToken(token, 'git_bridge'),
      { code: 'token_invalid' }
    )
    ctx.PersonalAccessToken.findOne.resolves({
      _id: 'id',
      user_id: 'u',
      scopes: ['git_bridge'],
  ...
  it('lists safe token fields and supports revocation', async ctx => {
    ...
    ctx.PersonalAccessToken.find.returns({
      sort: sinon.stub().returns({ lean: sinon.stub().resolves([rec]) }),
    })
    ...
    sinon.assert.calledWith(ctx.PersonalAccessToken.deleteOne, {
      _id: 'id',
      user_id: 'u',
    })
    await ctx.TokenService.promises.revokeAllForUser('u')
    sinon.assert.calledWith(ctx.PersonalAccessToken.deleteMany, {
      user_id: 'u',
    })
  })
})
